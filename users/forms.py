...
class SettingsForm(forms.Form):
    first_name = forms.CharField(label="Name*", required=False, widget=forms.TextInput(attrs={"id":"first_name", "name":"first_name", "maxlength":"100",
                                                              'placeholder': 'First'}) )
    last_name = forms.CharField(required=False, widget=forms.TextInput(attrs={"id":"last_name", "name":"last_name", "maxlength":"100", 
                                                               'placeholder': 'Last'}) )
    email = forms.EmailField(label="Email", widget=forms.EmailInput(attrs={"placeholder":"example@gmail.com"}))

    gender = forms.ChoiceField(widget=forms.RadioSelect, choices=[('Male', 'Male'),('Female', 'Female')], label="Gender*", initial="Male", required=False)

    height = forms.FloatField(label="Height (In.)*", required=False, widget=forms.TextInput(attrs={'placeholder': '70', "class":"numbers"}),
                              validators=[MinValueValidator(36.0), MaxValueValidator(100.0)])

    weight = forms.FloatField(label="Weight (Lbs.)*", required=False, widget=forms.TextInput(attrs={'placeholder': '180', "class":"numbers"}),
